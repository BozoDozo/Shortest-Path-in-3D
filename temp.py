...
class Chenille_2D:
    def __init__(self, x0: int, y0: int, size: int, canva: Canvas, cote: int) -> None:
        ...
        for i in range(self.size):
            xy_xy = circle_to_oval(
                x0*self.cote+self.cote/2, y0*self.cote+self.cote/2, 0.125*self.cote)
            self.chenille_id.append((self.Canva.create_oval(
                *xy_xy, fill="Green", state="normal", tags="chenille")))
