...
def trace_beizier_quatre(x0, y0, x1, y1, x2, y2, x3, y3, it, debut):
    """
...
def trace_beizier(liste_points, it):
    ...
    for i in range(0, n_int, 3):
        ...
        trace_beizier_quatre(*liste_points[i], *liste_points[i+1],
                             *liste_points[i+2], *liste_points[i+3], it, 0)
    ...
    if(n != 4):
        ...
        # Trace sur OpenGL le dernier point de la courb


def voir_bezier(liste_point_bezier):
    for point in liste_point_bezier:
        x = point[0]
        y = point[1]
        glBegin(GL_POLYGON)
