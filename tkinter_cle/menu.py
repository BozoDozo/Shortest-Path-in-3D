...
def modif_parametre():
    ...
    tk.Scale(win_2, label="Borne supérieur des valeurs", from_=2, to_=100,
    ...
    tk.Scale(win_2, label="Seuil des d’obstacles", from_=0, to_=5,
...
def chemin(event=None):
    """
    cette fonction trouve le chemin en djikstra
    """
    global depart_id, arrivee_id, Canva, liste_dji
    # faire djikstra
    if algo_chemin == 0:
        liste_dji = djikstraa(Matrice, depart, arrivee)
        tracer_dijkstra(liste_dji, Canva)
    else:
        liste_dji = astar(Matrice, depart, arrivee)
        tracer_astar(liste_dji, Canva)
    # tracer djikstra tkinter
...
def animation(event=None):
    """
    calcul bezier et animation
    """
    global liste_dji, Canva, cercle_existe, save_dep_global, save_ar_global
    global cercle1_anim, cercle2_anim, cercle3_anim, cercle4_anim
    it = 100
    if cercle_existe == 0:
        trace_beizier(liste_dji, it, Canva)
    point = liste_point_bezier[0]
    save_dep_global = point
    save_i = point[1]
    save_j = point[0]
    save_i1 = point[1]
    save_j1 = point[0]
    ...
    save_j3 = point[0]
    cote = 40
    xy_xy = circle_to_oval(save_j+cote/2, save_i, 0.25*cote)

    if cercle_existe == 0:
        cercle1_anim = Canva.create_oval(*xy_xy, fill="blue")
        cercle2_anim = Canva.create_oval(*xy_xy, fill="white")
        cercle3_anim = Canva.create_oval(*xy_xy, fill="yellow")
        cercle4_anim = Canva.create_oval(*xy_xy, fill="red")
        cercle_existe = 1
    else:
        dx = (save_ar_global[0]-save_dep_global[0])*-1
        dy = (save_ar_global[1]-save_dep_global[1])*-1
        Canva.move(cercle1_anim, dy, dx)
        Canva.move(cercle2_anim, dy, dx)
        Canva.move(cercle3_anim, dy, dx)
        Canva.move(cercle4_anim, dy, dx)
    numero = 0

    t = 0
    while t < len(liste_point_bezier)-1:
        # pour cercle1
        i = liste_point_bezier[t][1]
        j = liste_point_bezier[t][0]
        save_ar_global = [i, j]
        dx = (save_i-i)*-1
        dy = (save_j-j)*-1
        save_i = i
        ...
        # cercle 2
        if t > 2:
            i = liste_point_bezier[t-2][1]
            j = liste_point_bezier[t-2][0]
            dx1 = (save_i1-i)*-1
            dy1 = (save_j1-j)*-1
            save_i1 = i
            save_j1 = j
            Canva.move(cercle2_anim, dy1, dx1)
        # cercle 3
        if t > 4:
            i = liste_point_bezier[t-4][1]
            j = liste_point_bezier[t-4][0]
            dx2 = (save_i2-i)*-1
            dy2 = (save_j2-j)*-1
            save_i2 = i
            save_j2 = j
            Canva.move(cercle3_anim, dy2, dx2)
        # cercle 3
        if t > 6:
            i = liste_point_bezier[t-6][1]
            j = liste_point_bezier[t-6][0]
            dx3 = (save_i3-i)*-1
            dy3 = (save_j3-j)*-1
            save_i3 = i
            save_j3 = j
            Canva.move(cercle4_anim, dy3, dx3)
...
bouton_animation = tk.Button(bottom_frame, text="animation",
                             state="disabled", width=20, height=5, command=animation)
