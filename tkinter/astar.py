...
def coutdistance(xa,ya,xb,yb):
    ...
    Y=Y*Y
    return sqrt(X+Y)
def astar(matrice,liste_square_map,somde,somear):
    """
    ...
    while somde!=somear:
            ...
            liste_voisin=clipping_voisin(liste_square_map, xa,ya, rayon=1)
...
def tracer_astar(liste,canva):
    for id in liste:

        canva.itemconfigure(id,fill="pink")
